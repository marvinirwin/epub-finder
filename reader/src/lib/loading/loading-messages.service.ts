...
export class LoadingMessagesService {
    ...
    constructor(
        ...
        this.loadingMessages$ = combineLatest(
            [
                documentRepository.isFetching$
                    .pipe(map(isFetching => isFetching ? 'Fetching library' : '')),
                openDocumentsService.aVirtualTabulationIsLoading$
                    .pipe(map(isTabulating => isTabulating ? 'Counting words in documents' : ''))
            ]
        ).pipe(
