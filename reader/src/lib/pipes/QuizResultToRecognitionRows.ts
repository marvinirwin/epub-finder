    ...
    map(
      ([scorePair, wordScheduleRowDict]): WordRecognitionRow => {
        const previousRecords =
          wordScheduleRowDict[scorePair.word]?.d.wordRecognitionRecords || [];
        const nextRecognitionRecord = SrmService.getNextRecognitionRecord(
          previousRecords,
          scorePair.grade
        );
        const inARow = <T>(array: T[], filterFunc: (v: T) => boolean): T[] => {
        ...
        const nextDueDate = () => {
          ...
          switch (correctRecordsInARow.length) {
            ...
            default:
              return add(Date.now(), { days: nextRecognitionRecord.interval });
          }
        };
