...
export class TranslationAttemptScheduleService
  ...
  constructor({
    ...
    this.indexedScheduleRows$ = combineLatest([
      ...
      map(
        ([selectedVirtualTabulations, translationAttempts, weightedVocab]) => {
          ...
          virtualDocumentTabulation.serializedTabulations.forEach(
            (serialzedTabulation) =>
              serialzedTabulation.segmentWordCountRecordsMap.forEach(
                (value, key) => {
                  if (
                    key.text.split("").find(isChineseCharacter) &&
                    key.text.length > 5
                  ) {
          ...
          Object.entries(translationAttempts).forEach(([key, value]) => {
            if (scheduleRows[key]) {
