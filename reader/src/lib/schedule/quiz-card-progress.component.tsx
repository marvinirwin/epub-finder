...
export const QuizCardProgress = ({ quizCard }: { quizCard: QuizCard }) => {
  ...
  const frequencyDocuments = [
    ...(
      useObservableState(m.frequencyDocumentsRepository.selected$) || new Map()
    ).values(),
  ];
  // Now list them
  return (
    <div>
      {frequencyDocuments.map((frequencyDocument: FrequencyDocument) => (
        <FrequencyDocumentInfo
...
export const FrequencyDocumentInfo = ({
  frequencyDocument,
}: {
  frequencyDocument: FrequencyDocument;
}) => {
    ...
    >
      {progress && (
        <Fragment>
          ...
          <div>
            Unrecognized:{" "}
            <span className={unrecognizedCount}>
              {progress.readabilityState.unrecognized.length}
            </span>
          </div>
          <div>
            Somewhat Recognized:{" "}
            <span className={somewhatRecognizedCount}>
              {progress.readabilityState.somewhatRecognized.length}
            </span>
          </div>
          <div>
            Recognized:{" "}
            <span className={recognizedCount}>
              {progress.readabilityState.fullRecognition.length}
            </span>
          </div>
        </Fragment>
      )}
