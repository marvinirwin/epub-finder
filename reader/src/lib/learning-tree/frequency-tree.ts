...
*/
    ...
    constructor(
        ...
        const pushNodeToStack = (
            f: TabulatedFrequencyDocument,
        ): ds_Tree<TabulatedFrequencyDocument> => ({
            value: f,
            children: {},
            nodeLabel: f.frequencyDocument.name,
        })
        const stack: ds_Tree<TabulatedFrequencyDocument>[] = [
        ...
        while ((currentNode = stack.shift())) {
            const neighbors = this.getNClosestNeighbors(root, 3).map(
                pushNodeToStack,
            )
            currentNode.children = Object.fromEntries(
                neighbors.map((neighbor) => [neighbor.nodeLabel, neighbor]),
            )
            neighbors.forEach((neighbor) =>
                this.availableSet.delete(
                    neighbor.value as TabulatedFrequencyDocument,
                ),
            )
    ...
    private getNClosestNeighbors(
        root: TabulatedFrequencyDocument,
        n: number = 3,
    ) {
        const frequencyDocumentTabulationDistance = (
            tabulation: SerializedTabulation,
            tabulation2: SerializedTabulation,
        ) => {
        ...
        return orderBy([...this.availableSet.values()], (potentialChild) =>
            frequencyDocumentTabulationDistance(
                root.tabulation,
                potentialChild.tabulation,
            ),
        ).slice(0, n)
    }
}
