    ...
    _createClass(Sequence, [
        {
            ...
            value: function apply() {
                ...
                if (
                    pattern === null ||
                    instruments === null ||
                    durationPerStep === null
                ) {
                    return Tape.silence(0)
                }

                var durationPerStepList = Array.isArray(durationPerStep)
                    ? durationPerStep
                    : [durationPerStep]

                return pattern.split('').reduce(function (tape, ch, index) {
                    var instrument = getInstrumentFrom(
                        instruments,
                        ch,
                        index,
                        tape,
                    ...
                    var durationPerStep =
                        durationPerStepList[index % durationPerStepList.length]
                    ...
                    if (instrument !== null) {
                        if (instrument.duration < durationPerStep) {
                            tape = tape.concat(
                                instrument,
                                Tape.silence(
                                    durationPerStep - instrument.duration,
                                ),
                            )
                        } else {
                            tape = tape.concat(
                                instrument.slice(0, durationPerStep),
                            )
                        }
                    } else {
