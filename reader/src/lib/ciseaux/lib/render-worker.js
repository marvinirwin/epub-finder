...
self.onmessage = function (e) {
    switch (e.data.type) {
        case 'transfer':
            self.repository[e.data.data] = e.data.buffers.map(function (
                buffer,
            ) {
...
self.renderTrack = function (trackNum, fragments, destination, sampleRate) {
    ...
    for (var i = 0, imax = fragments.length; i < imax; i++) {
        ...
        var canSimpleCopy =
            ...
            !fragment.reverse &&
            srcCh <= dstCh &&
            srcSub[0].length === dstSub[0].length
...
self.process = function (src, dst, opts) {
    ...
    for (var i = 0; i < dstLength; i++, index += step) {
        ...
        for (ch = 0; ch < srcCh; ch++) {
            samples[ch] =
                src[ch][i0] + Math.abs(x0 - i0) * (src[ch][i1] - src[ch][i0])
        }
...
self.mix1['6->2'] = function (src) {
    return [
        src[0] + 0.7071 * (src[2] + src[4]),
        src[1] + 0.7071 * (src[2] + src[5]),
    ]
}
