  ...
  constructor({
    ...
    const applyListener = (element: HTMLElement) => {
      ...
      function highestPriorityMouseoverCard() {
        ...
        if (atomMetadata) {
          return priorityMouseoverHighlightWord({
            atomMetadata,
            cardsRepository,
          });
        }
      ...
      element.onclick = (ev) => {
        switch (mode()) {
          case Modes.VIDEO:
            ...
            const wordWithVideoData = atomMetadata.words.find((word) =>
              videoMetadataRepository.all$.getValue().get(word.word)
            );
    ...
    openDocumentsService.openDocumentTree
      ...
      .updates$.pipe(
        switchMap(({ sourced }) => {
          return merge(...flattenTree(sourced)).pipe(shareReplay(1));
        }),
