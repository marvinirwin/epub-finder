...
export type TemporaryHighlightRequest = {
    word: string
    color: RGBA
    duration: number
}
...
export class TemporaryHighlightService {
    public temporaryHighlightRequests$ = new ReplaySubject<
        TemporaryHighlightRequest | undefined
    >(1)
    ...
    constructor({
        ...
        highlighterService.timedHighlight(
            this.temporaryHighlightRequests$.pipe(
                // @ts-ignore
                filter(
                    (v: TemporaryHighlightRequest | undefined) =>
                        v !== undefined,
                ),
    ...
    public async highlightTemporaryWord(
        word: string,
        color: RGBA,
        duration: number,
    ) {
