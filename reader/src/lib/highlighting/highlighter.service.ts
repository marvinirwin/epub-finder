...
export class HighlighterService {
    ...
    public timedHighlight(
        ...
        combineLatest([
            ...
            ([timedHighlightDelta, currentHighlightMap, wordElementMap]) => {
    ...
    private removeHighlightDelta(
        ...
        oldHighlightDelta.forEach((colors, highlightTarget) => {
            const m: HighlightDelta[] | undefined = currentHighlightMap.get(
                highlightTarget,
            )
...
function updateElementBackgroundColor(
    ...
    const priorityLists = elementHighlightMap.get(
        elementToHighlight.element as HTMLElement,
    )
    const rgbas: RGBA[] = []
    if (priorityLists) {
        ...
        priorityLists.forEach((priorityList) => {
            const highestPriorityMapIndex = priorityList.findIndex(
                (highlightDelta) => highlightDelta && highlightDelta.size > 0,
            )
            if (highestPriorityMapIndex === highestPriority) {
                highestPriorityKeyValues.push(
                    ...priorityList[highestPriorityMapIndex],
                )
            } else if (
                highestPriorityMapIndex !== -1 &&
                highestPriorityMapIndex < highestPriority
            ) {
                highestPriority = highestPriorityMapIndex
                highestPriorityKeyValues = [
                    ...priorityList[highestPriorityMapIndex],
                ]
            }
        })
...
const computeElementHighlightMap = (
    ...
    wordElementMap.forEach((ltElements, targetToUpdate) => {
        ltElements.forEach((ltElement) => {
            const wordHighlightMapElement: HighlightDeltaPriorityList =
                wordHighlightMap.get(targetToUpdate) || []
            const element = (ltElement.element as unknown) as HTMLElement
            safePushMap(elementHighlightMap, element, wordHighlightMapElement)
        })
    })
