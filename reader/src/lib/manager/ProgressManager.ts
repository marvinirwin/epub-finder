...
export class HSKLevel {
  protected getWordScore(
    word: string,
    wordScores: Dictionary<WordRecognitionRow>
  ) {
  ...
  constructor(
    ...
    this.progress$ = this.wordScores$.pipe(
      map((wordScores) => {
    ...
    return combineLatest([
      this.wordRecognitionRows.pipe(
        ...
        map((wordRecognitionRows) =>
          Object.fromEntries(
            Object.entries(wordRecognitionRows)
              .map(([word, rows]) => [
                word,
                rows.filter((row) =>
                  dateBetween(row.timestamp, startTime, endTime)
                ),
              ])
      ...
      map(
        ([wordRecognitionWords, weightedWords]: [
          ds_Dict<WordRecognitionRow[]>,
          ds_Dict<WeightedWord>
        ]) => {
          const missingWords = new Set<string>(Object.keys(weightedWords));
          Object.entries(wordRecognitionWords).forEach(
            ([word, recognitionRows]) => {
              // If the last one is positive, we're good
              const correctToday =
                recognitionRows[recognitionRows.length - 1].grade >=
                CORRECT_RECOGNITION_SCORE;
              if (correctToday) {
          ...
          return [
            sum(
              Object.entries(weightedWords)
                .filter(([word, weight]) => !missingWords.has(word))
                .map(([word, weight]) => weight)
            ),
            sum(Object.values(weightedWords).map(([word, weight]) => weight)),
          ];
        }
      )
    );
