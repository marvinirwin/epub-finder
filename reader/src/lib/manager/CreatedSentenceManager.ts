...
export class CreatedSentenceManager {
  ...
  constructor(public db: DatabaseService) {
    ...
    this.allCreatedSentences$ = this.addPersistedCreatedSentence$.pipe(
      scan(
        (
          createdSentenceMap: Dictionary<CreatedSentence[]>,
          newCreatedSentences
        ) => {
          newCreatedSentences.forEach((createdSentence) => {
            if (createdSentenceMap[createdSentence.learningLanguage]) {
              createdSentenceMap[createdSentence.learningLanguage].push(
                createdSentence
              );
            } else {
              createdSentenceMap[createdSentence.learningLanguage] = [
                createdSentence,
              ];
            }
          });
