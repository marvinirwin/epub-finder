...
export default class EditingCardManager {
    showEditingCardPopup$: ReplaySubject<boolean> = new ReplaySubject<boolean>(
        1,
    )
    queEditingCard$: ReplaySubject<EditingCard | undefined> = new ReplaySubject<
...
*/
        this.editingCardIsSaving = this.editingCard$.pipe(
            switchMap((c) => (c ? c.saveInProgress$ : of(undefined))),
            shareReplay(1),
        )
        this.queEditingCard$
            .pipe(
                withLatestFrom(
                    this.showEditingCardPopup$.pipe(startWith(false)),
                ),
            )
