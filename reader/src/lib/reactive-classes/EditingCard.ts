...
*/
    ...
    saveData$
      .pipe(
        switchMap((d) => race(timer(1000), this.cardClosed$).pipe(mapTo(d)))
      )
      .subscribe(
        async ([photos, sounds, english, frontPhotos, characters, deck]) => {
          const iCard: ICard = {
            id: this.id,
            deck,
            photos,
  ...
  static fromICard(
    iCard: ICard,
    persistor: IndexDBManager<ICard>,
    c: CardsRepository
  ): EditingCard {
    const e = new EditingCard(persistor, c);
