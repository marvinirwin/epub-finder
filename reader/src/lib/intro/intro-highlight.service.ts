  ...
  constructor({
    ...
    renderedSegments$
      ...
      .subscribe(async (atomizedSentences) => {
        const allSentences = atomizedSentences
          .slice(0, 10)
          .map((atomizedSentence) => atomizedSentence.translatableText);

        function getRandomWords() {
          return allSentences.map((sentence) =>
            sentence.slice(...randomRange(0, sentence.length, 3))
          );
        }
...
const randomRange = (
  ...
  return [
    start,
    Math.min(
      Math.floor(start + Math.random() * endRange + 1),
      start + maxRangeSize
    ),
  ];
};
