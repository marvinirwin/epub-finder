...
export class DeltaScanner<T, U extends string = string> {
    ...
    constructor() {
        ...
        this.updates$ = this.appendDelta$.pipe(
            ...
            scan(
                (
                    scan: DeltaScan<T, U> | undefined,
                    delta: ds_Tree<T, U> | undefined,
                ) => {
                    if (!scan)
                        return {
                            sourced: delta,
    ...
    mapWith<U>(mapFunc: DeltaScanMapFunc<T, U>): DeltaScanner<U> {
        ...
        this.updates$.pipe(take(1)).subscribe(({ sourced }) => {
            if (sourced) {
                derivedTree.appendDelta$.next(MapTree(sourced, mapFunc))
            }
        })
        this.updates$.pipe(skip(1)).subscribe(({ delta }) => {
            derivedTree.appendDelta$.next(MapTree(delta, mapFunc))
        })
        return derivedTree
    }
...
export class NamedDeltaScanner<
    T extends Named,
    U extends string = string
> extends DeltaScanner<T, U> {
...
function MapTree<T, U>(
    node: ds_Tree<T>,
    mapFunc: DeltaScanMapFunc<T, U>,
): ds_Tree<U> {
    const newChildren = Object.fromEntries(
        Object.entries(node.children || {}).map(([nodeLabel, child]) => {
            return [nodeLabel, MapTree(child, mapFunc)]
        }),
    )
...
export function flattenTree<T, U extends string = string>(
    ...
    Object.values(tree.children || {}).forEach((child) =>
        flattenTree(child, a, filter),
    )
    return a
}

export function getElementByKeyPath<T, U extends string = string>(
    tree: ds_Tree<T, U>,
    keyPath: U[] = [],
): T {
...
export function getDeletedValues<T>(
    tree: ds_Tree<T> | undefined,
    delta: ds_Tree<T>,
    a: T[] = [],
): T[] {
    if (delta.delete) {
    ...
    if (delta.children) {
        Object.entries(delta.children).forEach(([key, child]) =>
            getDeletedValues(
                tree?.children?.[key],
                // @ts-ignore
