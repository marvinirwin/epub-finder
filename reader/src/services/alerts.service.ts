...
export class AlertsService {
  public static pipeToColor = (color: Color) => (
    o$: Observable<string>
  ...
  constructor() {
    ...
    axios.interceptors.response.use(
      ...
      (error) => {
        ...
        const msg =
          error?.response?.data?.message || error?.response?.statusCode;
        if (msg) {
  ...
  private appendAlertMessage(msg: string, color: Color) {
    ...
    const sliceStart =
      messages.length - MAX_MESSAGES > 0 ? messages.length - MAX_MESSAGES : 0;
    this.alertMessages$.next(
      messages
        .concat({ msg, severity: color })
        .slice(sliceStart, sliceStart + MAX_MESSAGES)
    );
  }
