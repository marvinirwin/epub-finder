...
export class HighlightPronunciationVideoService {
  constructor({
    ...
    const elementsToHighlight$ = pronunciationVideoService.videoPlaybackTime$.pipe(
      ...
      map(([playbackTimeMs, metadata]) => {
        ...
        return metadata.characters.findIndex(
          (char) => playbackTimeMs < char.timestamp * metadata.timeScale
        );
      }),
      distinctUntilChanged(),
      withLatestFrom(
        wordMetadataMapService.visibleWordSegmentMap,
        pronunciationVideoService.videoMetadata$
      ),
      map(([characterIndex, visibleSentences, videoMetadata]) => {
        ...
        visibleSentences
          .get(videoMetadata.sentence)
          ?.forEach((atomizedSentence) => {
            return delta.set(
              (atomizedSentence.element.childNodes[
                characterIndex
    ...
    highlighterService.singleHighlight(elementsToHighlight$, [
      0,
      "PRONUNCIATION_VIDEO_HIGHLIGHT",
    ]);
  }
}
