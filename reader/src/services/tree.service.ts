 ...
 * @param diffTree
 */
export function applyTreeDiff<T>(
    oldTree: ds_Tree<T> | undefined,
    diffTree: ds_Tree<T> | undefined,
): ds_Tree<T> | undefined {
    ...
    const allChildKeys = uniq(
        Object.keys(oldTree.children || {}).concat(
            Object.keys(diffTree.children || {}),
        ),
    )
...
export const flattenTreeIntoDict = (key?: string) => <
    ...
    U extends string
>(
    obs$: Observable<DeltaScan<T, U>>,
): Observable<ds_Dict<T>> => {
    return obs$.pipe(
        map(({ sourced }) => {
            const libraryDocuments =
                key === undefined ? sourced : sourced?.children?.[key]
            return Object.fromEntries(
                libraryDocuments
                    ? flattenTree<T>(libraryDocuments).map((document) => [
                          document.name,
                          document,
                      ])
                    : [],
