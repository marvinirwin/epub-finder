...
export class TreeMenuService<T, U extends TreeMenuProps<any>> {
  ...
  constructor({ settingsService }: { settingsService: SettingsService }) {
    const itemAtDirectoryPath$: Observable<
      ds_Tree<TreeMenuNode> | undefined
    > = this.itemAtPath$(
      settingsService.directoryPath$.pipe(
        map((str) => str.split(".").filter((v) => v))
      )
    );
    const componentAtActionPath$: Observable<
      ds_Tree<TreeMenuNode> | undefined
    > = this.itemAtPath$(
      settingsService.componentPath$.pipe(
        map((str) => str.split(".").filter((v) => v))
      )
    );
    ...
    ]).pipe(map(([itemAtPath, path]) => !!itemAtPath && !!path.length));

    this.selectedComponentNode$ = componentAtActionPath$.pipe(
      map((itemAtPath) => itemAtPath?.value)
    );
    this.selectedDirectory$ = itemAtDirectoryPath$.pipe(
      map((itemAtPath) => itemAtPath?.value)
    );
    ...
    this.actionSelected$
      .pipe(withLatestFrom(this.tree.updates$))
      .subscribe(([actionPath, { sourced }]) => {
        if (sourced) {
          const action = walkTree<TreeMenuNode>(sourced, ...actionPath)?.value
            ?.action;
          if (action) {
            action();
          }
  ...
  private itemAtPath$(
    path$: Observable<string[] | undefined> | Observable<string[]>
  ) {
    return combineLatest([path$, this.tree.updates$]).pipe(
      map(([path, { sourced }]) => {
        if (!path?.length) {
          return sourced;
        }
