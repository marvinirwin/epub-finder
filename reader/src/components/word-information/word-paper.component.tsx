...
export const WordPaperComponent: React.FC<{ wordCard: WordCard }> = ({
  ...
  const scheduleRows =
    useObservableState(m.quizCardScheduleRowsService.indexedScheduleRows$) ||
    {};
    ...
    >
      ...
      {sortInfo && (
        <Fragment>
          ...
          <Typography variant={"h6"}>
            Date Weight:{" "}
            {round(sortInfo.dueDate.weightedInverseLogNormalValue, 2)}
          </Typography>
          <Typography variant={"h6"}>
            Length: {round(sortInfo.length.weightedInverseLogNormalValue, 2)}
          </Typography>
          <Typography variant={"h6"}>
            Sentence Priority:{" "}
            {round(sortInfo.sentencePriority.weightedInverseLogNormalValue, 2)}
          </Typography>
        </Fragment>
      )}
      <br />
