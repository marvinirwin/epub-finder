...
export const LoadingBackdrop = () => {
  ...
  const thingsInProgress = useObservableState(
    m.progressItemService.progressItems$
  );
  const classes = useStyles();
  return (
    <Backdrop className={classes.backdrop} open={!!thingsInProgress?.size}>
      <CircularProgress id={"global-loading-spinner"} color="inherit" />
      <Typography id={loadingBackdropTypography}>
        {[...(thingsInProgress?.values() || [])].map((t, i) => (
          <ProgressItemComponent key={i} progressItem={t} />
        ))}
      </Typography>
    </Backdrop>
  );
