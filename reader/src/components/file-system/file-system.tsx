...
export const BackButton: React.FC<{ directory: string[] }> = ({
  directory,
}) => {
  if (!directory.length) {
    return null;
...
export const AddLearningMaterial: React.FC<{
  files: FileItem[];
  directory: string[];
}> = ({ files, directory }) => {
  const directoryString = directory.join("/");
  const itemsInThisDirectory = files.filter(
    (file) => directoryFromFilePath(file.path) === directoryString
  );
  return (
    <GridList cellHeight={160} cols={3}>
      ...
      {itemsInThisDirectory.map((tile) => {
        return (
          <GridListTile key={fileNameFromFilepath(tile.path)} cols={1}>
            {tile.Component}
          </GridListTile>
        );
      })}
