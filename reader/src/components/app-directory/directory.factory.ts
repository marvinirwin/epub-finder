 ...
 * ]
 */
export type ArrayToTreeParams<T extends Named> =
  | [T, ...(T | ArrayToTreeParams<T>)[]]
  | [];
export const arrayToTreeChildren = <T extends Named>(
  ...array: ArrayToTreeParams<T>
): { [key: string]: ds_Tree<T> } => {
  ...
  const entries: [[string, ds_Tree<T>]] = [
    [root.name, { nodeLabel: root.name, value: root, newTree: true }],
  ];
  for (let i = 0; i < nextItems.length; i++) {
    ...
    if (Array.isArray(nextItem)) {
      ...
      entries.push([
        nextItem.name,
        { nodeLabel: nextItem.name, value: nextItem, newTree: true },
      ]);
    }
  }
...
export const arrayToTreeRoot = <T extends Named>(
  ...array: [T, ...(T | ArrayToTreeParams<T>)[]]
): ds_Tree<T> => Object.values(arrayToTreeChildren(...array))[0];
