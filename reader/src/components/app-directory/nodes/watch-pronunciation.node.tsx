...
import { WATCH_PRONUNCIATION } from 'languagetrainer-server/src/shared'
...
export function WatchPronunciationNode(m: Manager): TreeMenuNode {
    return {
        ...
        LeftIcon: () => {
            ...
            const className =
                mode === Modes.VIDEO ? 'video-mode-icon-on' : undefined
            return <PlayArrow id={'watch-mode-icon'} className={className} />
        },
        action: async () => {
