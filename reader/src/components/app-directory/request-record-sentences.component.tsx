...
export const RequestRecordingSentences = () => {
  ...
  const sentences =
    useObservableState(m.requestRecordingService.allRenderedSentences$) || [];
  const selectedSentenceMap = useObservableState(
    m.requestRecordingService.recordRequestSentences$
  );
  const alreadySubmittedSet = useObservableState(
  ...
  const toggleSentenceSelected = useCallback(
    (sentence) => {
      m.requestRecordingService.recordRequestSentences$.next(
        selectedSentenceMap &&
          new Map(
            selectedSentenceMap.set(
              sentence,
              !!selectedSentenceMap.get(sentence)
            )
          )
      );
    },
