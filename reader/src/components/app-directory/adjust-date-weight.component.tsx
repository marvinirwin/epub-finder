...
export const AdjustTranslationAttemptSentenceWeight = () => {
  const m = useContext(ManagerContext);
  const dateWeight =
    useObservableState(m.settingsService.translationAttemptSentenceWeight$) ||
    0;
  return (
    <ListItem>
      <Typography gutterBottom>
        How much being included in the first translation attempt sentence
        influence the flash card order
      </Typography>
      <Slider
        value={dateWeight}
        onChange={(_, value) => {
