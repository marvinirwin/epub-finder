      ...
      sentences$: combineLatest([
        ...
        map(([sentenceMap, currentWord, translationAttemptScheduleIndex]) => {
          const firstTranslationAttempt =
            Object.values(translationAttemptScheduleIndex)[0]?.d?.segmentText ||
            "";
          ...
          return uniq(
            orderBy(
              ...
              [
                (segmentText) =>
                  firstTranslationAttempt.includes(segmentText) ? 1 : 0,
                (v) => v.length,
              ],
              ["desc", "asc"]
            )
    ...
    this.quizCard = {
      ...
      hiddenFields$: combineLatest([
        ...
        map(([, manualFieldConfig]) => {
          ...
          // @ts-ignore
          return m[manualFieldConfig] || computeRandomHiddenQuizFields();
        }),
        shareReplay(1)
      ),
