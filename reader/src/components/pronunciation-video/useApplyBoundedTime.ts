...
export const useApplyBoundedTime = (
  ...
  const setVideoTime = useDebouncedFn(() => {
    if (videoElementRef && highlightBarPosition1Ms && highlightBarPosition2Ms) {
      videoElementRef.currentTime =
        orderedPoints(highlightBarPosition1Ms, highlightBarPosition2Ms)[0] /
        1000;
  ...
  useInterval(() => {
    if (videoElementRef && highlightBarPosition1Ms && highlightBarPosition2Ms) {
      const isOverLimit =
        videoElementRef.currentTime * 1000 > highlightBarPosition2Ms;
      const isBeforeBeginning =
        videoElementRef.currentTime * 1000 < highlightBarPosition1Ms - 100;
