...
export const CharacterTimingSectionComponent: React.FunctionComponent<{
    ...
    const [hoverBarFraction, setHoverBarFraction] = useState<
        number | undefined
    >(undefined)
    const [highlightBar, setHighlightBar] = useState<HTMLDivElement | null>()
    usePlaceHighlightBar(
        highlightBar,
        sectionContainer,
        highlightStartPosition,
        highlightEndPosition,
    ...
    const editingIndex = useObservableState(
        manager.editingVideoMetadataService.editingCharacterIndex$,
    )
    const editing =
        videoMetaData && editingIndex !== undefined && editingIndex >= 0
    ...
    const onDropOver = (
        ...
        if (editing) {
            const positionFraction =
                (dragClientX - containerLeft) / containerWidth
            const newTimestamp =
                positionFraction * chunkSizeMs + chunkSizeMs * sectionIndex
            manager.pronunciationVideoService.setVideoPlaybackTime$.next(
                newTimestamp,
            )
            manager.editingVideoMetadataService
                .setCharacterTimestamp(
                    videoMetaData,
                    editingIndex as number,
                    newTimestamp,
                )
                .then((metadata) => {
                    manager.pronunciationVideoService.videoMetadata$.next(
                        metadata,
                    )
                    manager.pronunciationVideoService.setVideoPlaybackTime$.next(
                        newTimestamp - 100,
    ...
    return (
        <div
            className={'character-timing-section-container'}
            ...
            onMouseMove={(ev) => {
                ...
                if (canvas) {
                    ...
                    const fraction =
                        (ev.clientX - canvasWidth.x) / canvas.clientWidth
            ...
            onMouseDown={(ev) => {
                if (canvas) {
                    const canvasRect = canvas.getBoundingClientRect()
                    onMouseDown(
                        (ev.clientX - canvasRect.x) / canvas.clientWidth,
                    )
                }
            }}
            onMouseUp={(ev) => {
            ...
            onDragOver={(ev) => {
                ...
                if (
                    ev.clientX &&
                    sectionContainer?.clientWidth &&
                    sectionContainer.clientLeft
                ) {
                    debouncedOnDropOver(
                        ev.clientX,
                        sectionContainer.getBoundingClientRect().x,
                        sectionContainer.clientWidth,
                    )
                }
            }}
        ...
        >
            ...
            <TemporalPositionBarComponent
                position={
                    hoverBarFraction
                        ? hoverBarFraction * canvasWidth + 24
                        : undefined
                }
                color={'blue'}
            />
            <TemporalPositionBarComponent
                position={
                    progressBarFraction
                        ? progressBarFraction * canvasWidth + 24
                        : undefined
                }
                color={'black'}
            />
            <div
                ...
                className={'character-timing-section'}
            >
                {characterTimings.map((videoCharacter, index) => {
                    ...
                    if (editMode) {
                        props.onClick = (ev: React.MouseEvent<HTMLElement>) => {
                            ev.preventDefault()
                            ev.stopPropagation()
                            manager.editingVideoMetadataService.editingCharacterIndex$.next(
                                index + characterIndexStart,
                            )
                        }
                    }
                    return (
