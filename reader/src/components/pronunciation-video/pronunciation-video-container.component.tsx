...
export const PronunciationVideoContainer: React.FunctionComponent<{
    ...
    const videoTimeMs = useObservableState(
        m.pronunciationVideoService.videoPlaybackTime$,
    )
    ...
    useSubscription(m.browserInputsService.getKeyDownSubject('Escape'), () =>
                ...
                {chunkedCharacterTimings &&
                    ...
                    chunkedCharacterTimings.map(
                        (chunkedCharacterTiming, lineIndex: number) => {
                            ...
                            return (
                                <CharacterTimingSectionComponent
                                    ...
                                    highlightStartPosition={
                                        highlightBarPoints?.[0] || 0
                                    }
                                    highlightEndPosition={
                                        highlightBarPoints?.[1] || 0
                                    }
                                    audioBuffer={
                                        chunkedAudioBuffers?.[lineIndex]
                                    }
                                    onClick={(fraction) => {
                                        setReplayDragInProgress(false)
                                        window.clearTimeout(
                                            startDragTimeout || undefined,
                                        )
                                        if (mouseDownTime) {
                                    ...
                                    onMouseOver={(fraction) => {
                                        if (replayDragInProgress) {
                                            setHighlightBarMsP2(
                                                section.highlightBarNewPosition(
                                                    fraction,
                                                ),
                                            )
                                        }
