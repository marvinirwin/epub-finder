...
*/
    ...
    return (
        <div className={'documentstats'}>
            <div>
                Distinct Characters: {Object.keys(characterCounts).length}
            </div>
            <div>
                HSK-1 Characters:{' '}
                {
                    Object.keys(characterCounts).filter((character) =>
                        hsk1Characters.has(character),
                    ).length
                }
            </div>
            {orderBy(
                ...
                <div key={char}>
                    {char}: {count} HSK1:{' '}
                    {hsk1Characters.has(char) ? 'True' : 'False'}
                </div>
            ))}
        </div>
    )
