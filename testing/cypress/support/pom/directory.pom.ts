...
export class DirectoryPom {
    ...
    public static SubmitManualSpeechRecognition(result: string) {
        cy.get('#manual-speech-recognition-input').type(result)
        cy.get('#submit-manual-speech-recognition').click()
    }

    public static SetIsRecording(isRecording: boolean) {
        if (isRecording) {
            cy.get('#manual-is-recording').check()
        } else {
            cy.get('#manual-is-recording').uncheck()
        }
    }
    ...
    public static Back() {
        cy.get('#tree-menu-node-back-button').click()
    }
    ...
    public static SetDailyGoal(n: number) {
        ...
        cy.get(s).click().focused().clear().type(`${n}`)
    }
    ...
    public static EnterSpeechPractice() {
        return cy.get('#speech-practice').click()
    }
