...
async function tabulateChineseSentence(chineseSegment: string) {
    ...
    return tabulate({
        ...
        segments: AtomizedDocument.atomizeDocument(
            InterpolateService.sentences([chineseSegment]),
        ).segments(),
...
describe('document tabulation', () => {
    ...
    it('tabulates sentences with vocab', async () => {
        ...
        // @ts-ignore
        expect(
            [...tabulation3.segmentWordCountRecordsMap.values()][0],
        ).toHaveLength(26)
        const tabulation = await tabulateChineseSentence(
            '让安禄山兼任平卢、范阳、河东三镇节度使，就属于 唐玄宗制度上的错误。',
        )
        // @ts-ignore
        expect(
            [...tabulation.segmentWordCountRecordsMap.values()][0],
        ).toHaveLength(26)
        const tabulation2 = await tabulateChineseSentence(
            '陛下为何要爱惜此人，而亏损王法呢',
        )
        // @ts-ignore
        expect(
            [...tabulation2.segmentWordCountRecordsMap.values()][0],
        ).toHaveLength(13)
    })
})
