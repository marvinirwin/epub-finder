...
const converterOutputKeyId =
    process.env.DOCUMENT_CONVERTER_OUTPUT_S3_ACCESS_KEY_ID;
const converterOutputSecretKeyId =
    process.env.DOCUMENT_CONVERTER_OUTPUT_S3_ACCESS_KEY_SECRET;
...
export async function copyS3WithExtension(
    file: {
        originalname: string;
        bucket: string;
        key: string;
...
export const uploadToS3 = (content: Buffer) =>
    s3
        .upload({
            Bucket: inputConfig.bucket,
            Key: uuidv4(),
