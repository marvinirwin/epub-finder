...
import { entirelyDisabledService, wrapService } from "../util/ServiceWrapper";
...
const ENABLE_IMAGE_SEARCH = "ENABLE_IMAGE_SEARCH";
const client = wrapService({
  key: ENABLE_IMAGE_SEARCH,
  realService: () => new ImageSearchClient(new CognitiveServicesCredentials(
      imageSearchKey
    ),
    {
  ...
  mockService: entirelyDisabledService<ImageSearchClient>(ENABLE_IMAGE_SEARCH)
});
...
export class ImageSearchService {
  ...
  async fetchSearchResults({
                             term
                           }: ImageSearchRequestDto): Promise<ImageObject[]> {
  ...
  async lookupCacheEntry(
    imageSearchRequestDto: ImageSearchRequestDto
  ): Promise<ImageSearchRequestDto | undefined> {
